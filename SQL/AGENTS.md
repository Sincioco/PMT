...
- PMT Database Version 1.23 is the deployed BDO baseline as of July 20, 2026. The current source schema and rebuild scripts are Version 1.25 until that migration chain is deployed everywhere.
- Do not add new pre-1.23 compatibility logic. Every future SQL, database-contract, or database-backed stability change must include an explicit forward migration from Version 1.23 or the immediately preceding deployed version under `SQL/Migrations/`; changing rebuild scripts alone is not enough.
- The completed Version 1.22-to-1.23 migration, combined runner, and deployment runbook are historical artifacts under `Migrations/Migration History/`. The active forward migration chain is `Migrations/PMT_1.23_to_1.24.sql` then `Migrations/PMT_1.24_to_1.25.sql`, with `Migrations/PMT_1.23_to_1.25_All.sql` as the operator runner.
