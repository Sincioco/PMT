...
The deployed baseline, current source schema, and fresh rebuild scripts all represent Version 1.23. Version 1.23 adds current-month vacation examples for shared PMT/LMS/HLS demo members, selected-date On Behalf Of attendance, audited removal of explicit attendance entries, Development reset behavior that no longer refuses private content, shared-default plus per-user image-annotation template libraries stored as versioned JSON, persistent manual Diagram hierarchy ordering, clears all existing Documentation and Diagram pins while pinning is disabled, and seeds the unpinned public `PMT's Database Schema` demo Diagram. The seeded 13-template catalog is available to every user, personal overrides still synchronize across browsers and computers, embedded template image data remains in SQL, and the feature creates no upload-folder assets for Maintenance to collect. Diagram order uses `[pmt].[Blogs].[SortOrder]`; new Diagrams start at order `0` so Latest remains the normal baseline, while `[pmt].[MoveBlog]` moves an owner-scoped Diagram and rewrites only the supplied sibling order. The demo Blog body references the cache-versioned bundled `/assets/docs/pmt-database-schema.svg` asset instead of storing the large editable SVG in SQL; it contains all 29 current `[pmt]` tables and their field-to-field foreign-key mappings, and its `Entity Descriptions` layer is visible initially and can be hidden as one group. `PMT_SecurityRoleDefaultsVersion` remains `1.10`. Do not manually edit either version property.
...
There is currently no active forward migration because the source schema and every known deployed instance are at Version 1.23. The next database-affecting or database-backed stability change must add `PMT_1.23_to_1.24.sql` here and update the source/rebuild scripts in the same change. Do not edit the completed Version 1.23 migration to introduce new behavior.
