...
As of July 24, 2026, PMT Database Version 1.26 and its matching application release are deployed to BDO Production and every other known PMT instance. Version 1.26 is the deployed baseline.

Future migrations start from Version 1.26 or the immediately preceding deployed version. New compatibility work for versions before 1.26 is not required. Keep released migrations, combined runners, and deployment runbooks through Version 1.26 under `Migration History/` as immutable release artifacts.

The deployed baseline remains Version 1.26. The current source schema represents Version 1.27. Version 1.27 adds user Suggestions, GUID-token public Document and Diagram links with optional expiration, and linked Dev/Bug rollback behavior when QA has not touched the linked Bug. Version 1.26 adds four public PMT tutorial Documentation pages for Mentions and Live Cards, Diagrams, ERDs, and Image Annotations. Version 1.25 adds shared game-score leaderboard storage for PMT games. Version 1.24 refreshes the seeded public `PMT's Database Schema` demo Diagram to the current bundled `/assets/docs/pmt-database-schema.svg` asset, which uses the shared Diagram Entity spacing/routing rules, and soft-deletes duplicate seeded schema Diagram rows if any exist. Version 1.23 added current-month vacation examples for shared PMT/LMS/HLS demo members, selected-date On Behalf Of attendance, audited removal of explicit attendance entries, Development reset behavior that no longer refuses private content, shared-default plus per-user image-annotation template libraries stored as versioned JSON, persistent manual Diagram hierarchy ordering, cleared existing Documentation and Diagram pins while pinning is disabled, and seeded the unpinned public database-schema demo Diagram. `PMT_SecurityRoleDefaultsVersion` remains `1.10`. Do not manually edit either version property.

## Completed Migrations Through Version 1.26

All completed migration steps, combined runners, and deployment runbooks through Version 1.26 are stored under `Migration History/`. This includes the canonical Version 1.23-to-1.24, Version 1.24-to-1.25, and Version 1.25-to-1.26 steps plus their combined operator runners.

Do not rerun Version 1.26 or any earlier historical migration on a current Version 1.26 installation. Never run rebuild, create, stored-procedure, or seed scripts against an existing BDO database. Those scripts are for fresh disposable databases, not production upgrades.
...
Run `PMT_1.26_to_1.27_All.sql` to upgrade an existing Version 1.26 database to the current Version 1.27 source schema. It currently includes the single canonical `PMT_1.26_to_1.27.sql` step so deployment still uses the same one-file SQLCMD entry point.
