...
The deployed baseline is Version 1.22. The current source schema and fresh rebuild scripts represent Version 1.23, with `PMT_1.22_to_1.23.sql` as the canonical Production step and `PMT_1.22_to_1.23_All.sql` as the operator entry point. Version 1.23 also adds current-month vacation examples for shared PMT/LMS/HLS demo members, selected-date On Behalf Of attendance, audited removal of explicit attendance entries, Development reset behavior that no longer refuses private content, shared-default plus per-user image-annotation template libraries stored as versioned JSON, persistent manual Diagram hierarchy ordering, clears all existing Documentation and Diagram pins while pinning is disabled, and seeds the unpinned public `PMT's Database Schema` demo Diagram. The seeded 13-template catalog is available to every user, personal overrides still synchronize across browsers and computers, embedded template image data remains in SQL, and the feature creates no upload-folder assets for Maintenance to collect. Diagram order uses `[pmt].[Blogs].[SortOrder]`; new Diagrams start at order `0` so Latest remains the normal baseline, while `[pmt].[MoveBlog]` moves an owner-scoped Diagram and rewrites only the supplied sibling order. The demo Blog body references the cache-versioned bundled `/assets/docs/pmt-database-schema.svg` asset instead of storing the large editable SVG in SQL; it contains all 29 current `[pmt]` tables and their 82 foreign-key mappings, and its `Entity Descriptions` layer is visible initially and can be hidden as one group. `PMT_SecurityRoleDefaultsVersion` remains `1.10`. Do not manually edit either version property.
...
Run `PMT_1.22_to_1.23_All.sql` once from this directory. Its single included canonical step atomically renames the deployed `PMT` Project to `PMTQA`, recreates missing demo identities with non-public passwords, restores the original SQL-seeded `PMT` demo, adapts its Bugs to active BDO lookup values, adds non-overlapping current-month vacation examples for shared PMT/LMS/HLS demo members, updates `[pmt].[RecordAttendance]` for selected on-behalf dates, adds audited `[pmt].[RemoveAttendance]`, creates `[pmt].[ImageAnnotationDefaultTemplateLibraries]` plus `[pmt].[UserImageAnnotationTemplateLibraries]` with their get/save procedures, adds persistent Diagram hierarchy order through `[pmt].[Blogs].[SortOrder]` plus `[pmt].[MoveBlog]`, clears all existing Blog pins, and non-destructively inserts the unpinned public database-schema Diagram when Sin does not already own an active Diagram with that title. Existing Blog rows are preserved and receive the non-destructive default order `0`; newly created rows also start at `0`, and no existing Documentation body, title, privacy, Project, Sprint, or parent value is replaced. The migration loads the 13 shared defaults, stores an optional versioned override per user, limits each library to 50 MiB, and cascades personal cleanup when that user is deleted. Existing active vacation ranges are preserved and satisfy the demo example instead of receiving an overlapping duplicate. The migration also updates the post-deployment Development tools: broad Project cleanup protects only code `PMT`, Clear Users remaps private ownership to Sin, and Factory Reset may replace all data including private content. The one-step `_All.sql` wrapper is intentional so the tested entry point matches server execution; do not run the canonical step separately.
