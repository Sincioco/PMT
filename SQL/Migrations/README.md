...
`PMT_1.15_to_1.16.sql` is the canonical attendance/vacation step. `PMT_1.16_to_1.17.sql` adds opaque edit-version columns, focused collision procedures, aggregate write locks, and safe concurrent WFH-row initialization without changing existing business values. `PMT_1.17_to_1.18.sql` allows authorized non-administrators to pin their own Scrum entries without changing existing rows or private Personal Log pinning. `PMT_1.18_to_1.19.sql` preserves existing audit rows while backfilling their effective user as the actor and installs the server-controlled impersonation/Audit Trail contract. `PMT_1.19_to_1.20.sql` changes only the exact known seed Documentation rows to public, preserves all other private Documentation, performs the one-time correction of legacy generated-prefix Sprint and work-item codes under the PMT Project, and installs the transactional Project-code synchronization procedure. The PMT repair uses `ProjectId` as authority, leaves already-correct codes unchanged, includes deleted historical rows, and assigns the next available normal PMT code only when a legacy suffix collides. `PMT_1.20_to_1.21.sql` preserves existing data while replacing the old RCA append/bidirectional rule with Dev Task-to-Bug replacement, copying Bug URLs to linked Bug Fix Dev Tasks, and preventing Developer-role users from saving Dev Task statuses ordered after `QA Passed`. `PMT_1.21_to_1.22.sql` records successful password and invitation logins without advancing `Users.RowVersion`, exposes those timestamps to User cards, and stops administrator access from replacing the user's configured Role; exact legacy `Admin` Role values are changed to the prior `Developer` fallback because no separate title survived the old overwrite. Because BDO starts at Version 1.15 and this release contains seven steps, the deployment operator must run `PMT_1.15_to_1.22_All.sql` from this directory in SQLCMD mode and let it finish successfully before starting the matching Version 1.22 application. Do not run the seven step scripts manually during the normal deployment. Superseded combined runners through Version 1.21 are retained under `Migration History/`.

### Recovery after a failed Version 1.20 attempt

The combined runner commits each completed version step before starting the next one. If Version 1.20 fails, its data transaction and version update roll back, but the database may remain at Version 1.19 with the Version 1.20 procedure definitions partially installed. Check `PMT_DatabaseVersion` after restoring or recovering the database; do not choose a runner by assumption.

- For a database restored to Version 1.15, run only `PMT_1.15_to_1.22_All.sql`.
- For a database still at Version 1.19 after the failed attempt, run only `PMT_1.19_to_1.22_All.sql`.

Both files are one-command SQLCMD runners. Do not manually change the version property, delete a collision row, or run the individual remaining scripts one by one.
...
If one deployment requires two or more versioned migrations, always add a combined `PMT_<start>_to_<end>_All.sql` SQLCMD runner. Keep the version-step files canonical, and make the combined file use `:on error exit` plus ordered relative `:r` includes. For the active seven-step chain above, `PMT_1.15_to_1.22_All.sql` is the normal operator-facing runner. `PMT_1.19_to_1.22_All.sql` is the recovery runner for a database confirmed to have been left at Version 1.19 by the failed Version 1.20 attempt. The deployment operator runs exactly one applicable combined file instead of running the constituent files one by one.
