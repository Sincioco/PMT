...
Run `PMT_1.26_to_1.27_All.sql` to upgrade an existing Version 1.26 database to the current Version 1.27 source schema and insert the public `PMT Field Mapping Example` Diagram without deleting or replacing unrelated content. It currently includes the single canonical `PMT_1.26_to_1.27.sql` step so deployment still uses the same one-file SQLCMD entry point.
