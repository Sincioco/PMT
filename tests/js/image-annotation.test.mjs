...
test("RTE Insert Linked Diagram stores a database-backed Diagram OLE reference", async () => {
  ...
  assert.match(appSource, /bindRichDiagramOleViewer\(block, diagram\);[\s\S]*bindRichDiagramOleResizePersistence\(block, diagram\);[\s\S]*return;/);
  ...
  assert.match(formsCss, /\.pmt-diagram-ole:has\(\.pmt-diagram-ole-viewport:hover\)/);
  assert.match(formsCss, /\.pmt-diagram-ole::after/);
