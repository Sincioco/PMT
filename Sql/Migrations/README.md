...
The current source schema is Version 1.10. Apply these migrations in order to upgrade the deployed Version 1.3 database:
...
The BDO baseline is not considered upgraded until all seven migrations are actually deployed and verified.

For the July 14 BDO deployment, run `PMT_1.3_to_1.10_All.sql` from this folder in SQLCMD mode. It is a small one-command wrapper that applies the seven canonical versioned migrations above in order and stops on the first error. Keep all eight files together.
