...
The completed July 24, 2026 BDO deployment established PMT Database Version 1.26 as the baseline for every known instance. The completed migrations, combined runners, and deployment runbooks through Version 1.26 are retained under `SQL/Migrations/Migration History/`; do not rerun them on a current installation. Future deployment runbooks must begin at Version 1.26 or the immediately preceding deployed version and use a combined migration runner whenever a release spans more than one versioned migration.

The current source release represents Database Version 1.27 while BDO Production remains at Version 1.26. Before starting the matching Version 1.27 application binary, run `SQL/Migrations/PMT_1.26_to_1.27_All.sql` in SQLCMD mode. Do not run the individual migration directly during deployment; the combined runner is the one-file operator entry point.
