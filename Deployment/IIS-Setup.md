...
The completed July 28, 2026 deployment established PMT Database Version 1.27 as the baseline for BDO Production and every other known instance. The completed migrations, combined runners, and deployment runbooks through Version 1.27 are retained under `SQL/Migrations/Migration History/`; do not rerun them on a current installation. Future deployment runbooks must begin at Version 1.27 or the immediately preceding deployed version and use a combined migration runner whenever a release spans more than one versioned migration.

The current source release is Version 1.28 while every known deployed database remains at the Version 1.27 baseline. Version 1.28 does not change tables, stored procedures, seed data, or user rows; it advances the guarded database release marker. Stop the application, verify the backup, and run `SQL/Migrations/PMT_1.27_to_1.28_All.sql` in SQLCMD mode before deploying the matching application files. Follow `SQL/Migrations/2026-07-31 - PMT - BDO Version 1.28 Deployment.html`.
