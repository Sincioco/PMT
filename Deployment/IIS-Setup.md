...
The current source release represents Database Version 1.18 and adds Scrum attendance/vacation storage, `ROWVERSION` save-collision protection, and authorized non-admin Scrum pinning in three ordered steps. For an existing Version 1.15 installation, back up and verify the database, keep IIS stopped, change to the `SQL\Migrations` directory, and run `PMT_1.15_to_1.18_All.sql` in SQLCMD mode before deploying or starting the matching Version 1.18 application. This combined runner applies all required version steps in order and stops on the first error. Never start the Version 1.18 binary against a Version 1.15, Version 1.16, or Version 1.17 schema.
