...
| `diagram2-editor-relationships.js` | Renderer-neutral relationship selection objects, relationship add/delete/style/type/routing plans, manual route override plans, route point add/remove plans, and Auto Format - Compact candidate planning. | SVG node patching, host prompts, worker lifecycle, or document persistence. |
| `diagram2-route-costing.js` | Renderer-neutral route scoring, no-worse comparison, deterministic tie-breaks, and Compact diagnostics. | Entity layout mutation, DOM/SVG painting, host prompts, or command history. |
| `diagram2-compact-engine.js` | Async Compact phases, progress callbacks, cancel checks, no-improvement handling, and final diagnostics. | Browser overlay rendering, document save, or renderer DOM patching. |
| `diagram2-compact-worker.js` | Worker-capable Compact message entry point for larger future promotion gates. | Current controller ownership, UI progress overlay, or canonical persistence. |
...
Phase 5 adds focused Entity, relationship, routing, route-costing, and Compact modules rather than putting all ERD behavior into the controller, shell, renderer, or host adapters. `diagram2-editor-entities.js` owns Diagram 1-compatible Entity parsing and renderer-neutral Entity command plans, including visible field row updates and relationship-safe field cleanup. `diagram2-editor-relationships.js` owns relationship command plans, relationship selection objects, style/default/manual-route plans, route point add/remove plans, and Auto Format - Compact candidate planning. `diagram2-routing.js` wraps the renderer-neutral Diagram 1 route model and exposes normalized route paths, bounds, type normalization, and route-handle adjustment helpers for Diagram 2.

The shared controller remains a coordinator: it creates undoable commands from these plans, supplies dirty object/relationship ids to the renderer, tracks canonical revision for stale-result rejection, and exposes the same methods to the top-navigation host and both RTE hosts. The host adapters own only dialogs, progress overlays, notifications, save/cancel boundaries, schema-generation callbacks, and host-specific refresh. The live renderer owns keyed relationship SVG nodes, hit paths, route handles, route preview paths, route indexes, cache signatures, and diagnostics, but it still does not own the canonical Entity or relationship model.

Phase 5 keeps the established KISS boundary for long-running layout work: Auto Format - Compact is explicit, global, progress-capable, cancelable, undoable as one command, preserves locked Entity positions, preserves manual route overrides, scores route quality before commit, rejects equal/worse candidates, and enables compact routing only when the chosen candidate improves the layout. A worker-capable entry point now exists, while the current controller uses the shared async engine directly. Phase 8 still owns the 500/1,000 Entity promotion gates and any decision to move Compact execution fully off the main thread.
