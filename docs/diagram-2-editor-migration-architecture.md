...
| `diagram2-editor-relationships.js` | Renderer-neutral relationship selection objects, relationship add/delete/style/type/routing plans, manual route override plans, and Auto Format - Compact command planning. | SVG node patching, host prompts, worker lifecycle, or document persistence. |
...
Phase 5 adds focused Entity, relationship, and routing modules rather than putting all ERD behavior into the controller, shell, renderer, or host adapters. `diagram2-editor-entities.js` owns Diagram 1-compatible Entity parsing and renderer-neutral Entity command plans. `diagram2-editor-relationships.js` owns relationship command plans, relationship selection objects, style/default/manual-route plans, and the explicit Auto Format - Compact plan. `diagram2-routing.js` wraps the renderer-neutral Diagram 1 route model and exposes normalized route paths, bounds, type normalization, and route-handle adjustment helpers for Diagram 2.

The shared controller remains a coordinator: it creates undoable commands from these plans, supplies dirty object/relationship ids to the renderer, and exposes the same methods to the top-navigation host and both RTE hosts. The host adapters own only dialogs, notifications, save/cancel boundaries, and host-specific refresh. The live renderer owns keyed relationship SVG nodes, hit paths, route handles, route preview paths, route indexes, cache signatures, and diagnostics, but it still does not own the canonical Entity or relationship model.

Phase 5 keeps the established KISS boundary for long-running layout work: Auto Format - Compact is explicit, global, undoable as one command, preserves locked Entity positions, preserves manual route overrides, and enables compact routing after commit. No Web Worker was added because the measured automated fixtures did not show a responsiveness need during this phase; Phase 8 remains the place for 500/1,000 Entity promotion gates, worker cancellation, and deeper production-load hardening.
