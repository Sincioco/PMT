...
- [ ] Click Rectangle, Arrow, Text Box, and Entity; confirm each tool immediately inserts one selected object. Cancel the Add Entity dialog and confirm its temporary Entity disappears.
...
- [ ] In both edit and read-only Diagram views, move the pointer away from every Entity and confirm the expand/collapse and Data Type header controls are hidden. Move near an Entity and confirm its controls appear and remain usable.
