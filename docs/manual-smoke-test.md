...
- [ ] Clear PMT browser preferences, log in, and confirm What's New shows only Days 35, 34, and 33 in a left navigation list with Day 35 selected.
...
- [ ] Choose `click here` in the footer and confirm the dialog closes, `#/release-notes` opens, and the latest release is at the top of the 33 available historical releases. Confirm no invented Day 10 or Day 11 entries appear because those source prompts do not exist.
...
- [ ] Set `pmt-release-notes-last-seen:{userId}` to `2026-07-14-day-27`, log in again, and confirm What's New shows only Days 35 through 28.
...
- [ ] Confirm Select, Crop, Rectangle, Arrow, Text Box, and Entity each use a distinct recognizable glyph and expose an accessible name with its keyboard shortcut. Click Rectangle, Arrow, and Text Box and confirm each click immediately inserts and selects a centered instance, then returns to Select automatically. Add wrapped text; confirm fill, outline, text color, font, font-size, Left/Center/Right horizontal alignment, and Top/Middle/Bottom vertical alignment controls work; and the exported image remains SVG.
...
- [ ] Draw visibly overlapping rectangles, an arrow, text, and a second image, then open Objects. Confirm the compact tree is the actual SVG paint stack: the highest/frontmost painted object is at the top and the lowest/backmost is at the bottom. Group several items and confirm one logical group row contains its member rows. Confirm Original Image and the second image use the same normal image controls and can each be reordered, grouped, hidden, selected, and deleted.
...
- [ ] Drag standalone rows, including Original Image, above and below one another and confirm one horizontal insertion line shows the exact destination while dragging, clears when the pointer leaves valid targets, and immediately repaints overlapping canvas objects in the matching SVG z-order without Apply or a dialog refresh. With a group already at the top, drag a standalone object and a second group above that group header; confirm the line appears above the header and each drop creates the new top root layer instead of adding members to the group. Drop below a group header and confirm the object becomes a child; drag that child to the root and confirm it is ungrouped. Drag a group row and confirm all of its children move as one ordered block. Drop one group onto another and confirm PMT merges their members into one flat logical group instead of creating a nested group.
...
- [ ] Resize an image, rectangle, text box, Entity, and multi-object group from each corner handle; confirm the bounds remain proportional and contained text or arrows scale proportionally. Drag the top/bottom side handles and confirm only height changes; drag the left/right side handles and confirm only width changes while text size remains unchanged. For an Entity, confirm the PK/FK column width remains fixed while the field-name area widens.
- [ ] Hold Ctrl while using both corner and side resize handles. Confirm the selection center stays fixed while the corner remains proportional or the chosen side changes only its axis. Release Ctrl and confirm the next resize returns to the normal opposite-handle anchor.
...
- [ ] Drag an unlocked object while holding Shift and confirm it moves left/right only; repeat with Alt and confirm it moves up/down only. Release the modifier during the same gesture and confirm unconstrained movement resumes.
...
- [ ] Crop two different images to different bounds. In Objects, confirm each row independently shows Cropped and can toggle its own crop off and on without changing the other image. Click Crop again on one cropped image, choose Remove Crop, and confirm only that crop clears. Repeat on the other image, choose Apply Crop Permanently, accept the irreversible warning, and confirm only the retained pixels remain with a Permanent crop cue.
...
- [ ] Confirm Diagram is a top-level navigation item immediately before Release Notes. Open it and confirm the shared annotation editor starts without an Original image, with Grid and Snap off, and with a zoom dropdown containing 10% through 200% in 5% increments.
- [ ] In any RTE, place the caret between two known text markers and choose Insert Diagram. Cancel once and confirm no upload or image is inserted. Repeat, add a vector object, choose Insert Diagram, and confirm one editable Diagram image is inserted at the saved caret with no Original Image row. Reopen Edit Annotation and confirm the same canvas objects return.
- [ ] Click Rectangle, Circle, Arrow, Line, Text Box, and Entity; confirm each tool immediately inserts one selected object and Circle starts with equal width and height. Cancel the Add Entity dialog and confirm its temporary Entity disappears.
- [ ] Add `pmt.Projects` and `pmt.WorkTasks` by pasting SQL Server `CREATE TABLE` scripts that define PK, datatype, nullability, identity, and a WorkTasks-to-Projects FK. Confirm unneeded brackets are omitted, fields render correctly, and the relationship connects the mapped fields with the dependent arrowhead pointing to its parent/source field.
- [ ] Open Objects and confirm Entity Relationships remains one selectable parent while each relationship is also a selectable child. Change one child's line color/width/arrowhead, then format the parent and confirm individual and whole-diagram relationship styling, Undo/Redo, and template save/reuse work.
- [ ] Select the central Entity and run Auto Format - Org Tree. Confirm it stays at the top, direct children share a lower top-aligned row, Anchors do not move, and relationship paths neither cross an Entity nor attach generically to a table. Save and confirm PMT creates a private Documentation item containing the editable Diagram.
- [ ] In a dense schema, inspect a relationship whose tables are offset vertically. Confirm it uses the shortest clear orthogonal field-to-field route instead of looping around distant tables or the canvas edge.
- [ ] Select an Entity, open the Entity tab, scroll to its bottom, and choose Generate PMT Database Schema. Confirm the button disables while running and creates a separate private `PMT's Database Schema` Diagram containing every current `[pmt]` table and its field-to-field relationships without replacing the open Diagram.
- [ ] Edit the full PMT Database Schema and repeatedly zoom with Ctrl+wheel, toolbar buttons, and the percentage dropdown. Confirm zoom remains responsive, stays anchored at the pointer, and does not make relationship lines flicker or disappear. Cancel editing and confirm the read-only TreeNav viewport fits and re-centers the selected Diagram.
- [ ] In both edit and read-only Diagram views, move the pointer away from every Entity and confirm the expand/collapse and Data Type header controls are hidden. Move near an Entity and confirm its controls appear and remain usable. In edit mode, toggle Data Types and drag an Entity repeatedly; confirm its header, field names, and data types remain fully visible throughout each redraw.
- [ ] In read-only mode, expand Data Types on an Entity beside another table. Confirm the expanded Entity paints in front and any overlapping movable neighbor shifts aside without moving an Anchor table.
- [ ] In edit mode, drag or resize an Entity over another table and paste or duplicate an Entity near existing tables. Confirm movable Entities cascade aside until every table is fully visible; reopen read-only mode and confirm any pre-existing overlap is also resolved.
