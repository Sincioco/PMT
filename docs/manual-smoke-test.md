...
- [ ] Clear PMT browser preferences, log in, and confirm What's New shows only Days 30, 29, and 28 in a left navigation list with Day 30 selected.
...
- [ ] Choose `click here` in the footer and confirm the dialog closes, `#/release-notes` opens, and the latest release is at the top of the 28 available historical releases. Confirm no invented Day 10 or Day 11 entries appear because those source prompts do not exist.
...
- [ ] Set `pmt-release-notes-last-seen:{userId}` to `2026-07-14-day-27`, log in again, and confirm What's New shows only Days 30, 29, and 28.
...
- [ ] Drag standalone rows above and below one another and confirm the overlapping canvas objects immediately repaint in the matching SVG z-order without Apply or a dialog refresh. Drag a standalone object onto a group and confirm it becomes a child; drag that child to the root and confirm it is ungrouped. Drag a group row and confirm all of its annotation children move as one ordered block. Drop one group onto another and confirm PMT merges their members into one flat logical group instead of creating a nested group, while any grouped source image remains the fixed bottom layer.
