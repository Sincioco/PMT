...
- Development Project cleanup preserves and detaches every private Documentation and private Log row. `PMT` is the resettable demo Project and `PMTQA` is the retained BDO Project; broad cleanup protects both codes. Clearing users or restoring all initial seed data is refused while another user owns private content; a full rebuild performed directly in SQL remains an administrator-controlled database operation.
