...
- Draw vertical relationship trunks as close to the relevant table's protected margin as the route allows. The first trunk should sit just outside the yellow-zone margin, not arbitrarily far away. If a route needs more than one vertical trunk in the same corridor, offset each next trunk by one table-margin distance rather than creating arbitrary staggered zig-zags. Do not let arrow-head size inflate the table margin; the ERD margin is a stable readability spacing rule.
- Relationship lines must not fan out from an entity beyond the table-margin width. The short horizontal field stub may extend one margin unit to the first vertical lane; any additional parallel lanes should step outward by exactly one margin unit only when needed. A connector should never leap several margin widths away from the entity just to route around empty space.
- For a central table related to tables on both sides, route each relationship on the nearest useful side of the central table. A left-side child should connect to the central table's left field edge with a short stub and a compact local dogleg. A right-side child should connect to the central table's right field edge with its own short stub and compact dogleg. Do not force both relationships to share one outside perimeter lane when local left/right corridors are available.
- Auto Format - Compact should balance the selected central table's direct neighbors across the left and right sides whenever possible. This applies to parents/lookups and children. Do not put two or more same-level direct neighbors on the same side while the opposite side is empty; split them left/right in stable relationship-field order so the router is not forced to draw a giant perimeter rectangle. If there are two direct children, render one child on each side.
- "Compact" rendering means related tables are placed at the minimum permitted spacing allowed by the layout algorithm: close enough to save canvas space and read as one local relationship group, but not closer than the table margin/collision rules allow. If the distance were any smaller, PMT should move one or both tables apart. Running Auto Format - Compact should pull unanchored tables back into that compact spacing even if the user previously dragged them outward; Anchor table is the opt-out.
- Compact rendering still preserves level rows. Level 1 tables share the same top, level 2 tables share the same top, and so on; compact left/right side placement must not stagger children vertically just to align individual field anchors.
- When two related tables sit near each other vertically or diagonally, prefer routing through the space between them instead of fanning far around the outside. A relationship should not go up, across a large perimeter, and back down unless that is the only clear route.
- If the target field/table is below the source, prefer a downward local route after the short field stub; do not route upward first and then come back down unless every downward/local corridor is blocked. Apply the mirror rule when the target is above.
- The forbidden "red line" anti-pattern is a wraparound/perimeter route: a connector leaves a field, travels far to the outside of the related tables, runs down or across a long empty perimeter, then doubles back toward the target field. This creates big rectangular detours, tall outside trunks, and unnecessary S/U-shaped paths around a central entity. Do not render that style when a local corridor between the related tables is available.
- Another forbidden anti-pattern is a border-hugging trunk: after a short field stub, the connector turns and runs vertically or horizontally almost on top of the entity's outer border for a long distance. Long connector trunks must sit in the open corridor outside the entity margin, not ride beside the table outline where they look like a second border or visual fence.
- Relationship lines should respect a visual margin around unrelated entities. Tables should not overlap or nearly touch, and unrelated passing lines should not hug another table's border.
...
- An annotated RTE image stores both its current generated SVG URL and its active source upload URL in the owning record's rich-text HTML. The active source is normally the original full-resolution image. Applying a crop permanently replaces it with a PNG containing only the retained pixels; after the owning record is saved, the former source and former generated SVG are ordinary orphan-file candidates unless another record still references them. Maintenance must count the two current URLs as references; editable annotation state lives inside the referenced SVG rather than in an unlinked sidecar file.
...
- PMT, LMS, and HLS seed scripts explicitly create their 19 seed Documentation records as public. The private-by-default behavior for normal user-created Documentation remains unchanged.
...
Generate PMT Database Schema is a live schema read that requires Documentation/Create permission, not a clone of the seeded demonstration. It includes every current table in `[pmt]` and the database's current column order, datatypes, identity, nullability, PK, and field-to-field FK mappings. Each click creates a separate private editable Diagram named `PMT's Database Schema`; it cannot overwrite the Diagram currently being edited. PMT metadata access remains stored-procedure-only through `[pmt].[GetPmtDatabaseSchema]`.
...
The annotation Objects tree is another view of the live canvas and SVG paint stack. Its highest, frontmost painted item is at the top and its lowest, backmost item is at the bottom; dragging a row immediately rewrites that SVG paint order and repaints the canvas at the matching z-order. A single horizontal insertion line shows the pending root position. The upper half of a group header places an object or complete group before that group at the root, including above the current top group; the lower half intentionally moves it inside the group. Every canvas image is the same embedded-image object type. An RTE source starts as an object named Original Image, but it may be selected, hidden, grouped, reordered, cropped, or deleted like any other image. Each image owns its reversible crop, Cropped cue, and crop-visibility toggle. Selecting objects or groups in either the tree or canvas updates the same selection. Tree rename, multi-delete, pasted objects, and drag/drop changes participate in normal annotation history, while Copy only updates the editor's native clipboard.
