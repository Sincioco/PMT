...
Deployed databases and fresh rebuilds record `PMT_DatabaseVersion = 1.27`. There is no active forward migration. The completed `PMT_1.26_to_1.27.sql` step and `PMT_1.26_to_1.27_All.sql` operator runner are historical artifacts under `SQL/Migrations/Migration History/`. `PMT_SecurityRoleDefaultsVersion` remains `1.10`. Do not manually edit either version property.
