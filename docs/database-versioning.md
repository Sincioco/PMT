...
As of July 28, 2026, PMT Database Version 1.27 and its matching application release are deployed to BDO Production and every other known PMT instance. Version 1.27 is the deployed baseline. Protecting the data and stability of those instances is the top priority.

Future work does not need new upgrade compatibility for database versions before 1.27. Keep released migrations, combined runners, and deployment runbooks through Version 1.27 under `SQL/Migrations/Migration History/`. Start the next forward migration from Version 1.27; after a later version is deployed to every known instance, that version becomes the new baseline.

The current source tree and fresh-rebuild scripts represent Version 1.28. Version 1.28 retains the Version 1.27 table, stored-procedure, and seed contract and advances only the guarded database release marker for the frontend-only Diagram and rich-text editor release. Version 1.27 adds `[pmt].[Suggestions]` plus `[pmt].[AddSuggestion]` for user-submitted PMT suggestions, `[pmt].[PublicBlogLinks]` plus `[pmt].[CreatePublicBlogLink]` and `[pmt].[GetPublicBlog]` for GUID-based anonymous read-only public Document and Diagram links with optional expiration, an `[pmt].[UpsertTask]` rule that lets a developer pull a linked Dev Task back from Ready for QA to In Progress when QA has not touched the linked Bug, and the public editable `PMT Field Mapping Example` Diagram backed by the bundled `/assets/docs/pmt-field-mapping-example.svg` asset. The Diagram is present in fresh installs and was distributed to existing Version 1.26 installs by the completed Version 1.27 migration; its normal creation/update timestamp places it under the existing Latest sort without forcing a persistent selection. Version 1.26 adds public PMT tutorial Documentation pages for Mentions and Live Cards, Diagrams, ERDs, and Image Annotations to the permanent PMT seed and to existing installs through a forward migration. Version 1.25 adds `[pmt].[GameScores]` plus `[pmt].[AddGameScore]` and `[pmt].[GetGameScores]` so shared game leaderboards can persist scores by game key and signed-in PMT user. Version 1.24 keeps the deployed Version 1.23 schema contract and refreshes the seeded public `PMT's Database Schema` Diagram so its Blog body references the current cache-versioned bundled `/assets/docs/pmt-database-schema.svg` file and duplicate seeded schema Diagram rows are soft-deleted. Version 1.23 preserved BDO's former PMT Project as `PMTQA`, restored the original SQL-seeded `PMT` Project for demos, recreated missing demo identities without public default passwords, adapted demo Bug values to active BDO lookups, seeded current-month vacation examples for shared PMT/LMS/HLS demo members and 13 shared image-annotation templates for every user, persisted manual Diagram hierarchy order through `[pmt].[Blogs].[SortOrder]` and `[pmt].[MoveBlog]`, cleared existing Documentation and Diagram pins while pinning is disabled, seeded the unpinned public database-schema demo Diagram with all current `[pmt]` tables and foreign-key mappings, and exposed the connected database's current `[pmt]` catalog through `[pmt].[GetPmtDatabaseSchema]` for on-demand Diagram generation by users with Documentation/Create permission. The seeded Diagram SVG files remain outside SQL and do not create upload-folder files:
...
Deployed databases record `PMT_DatabaseVersion = 1.27`; fresh rebuilds record `PMT_DatabaseVersion = 1.28`. The active `PMT_1.27_to_1.28.sql` step and `PMT_1.27_to_1.28_All.sql` operator runner advance only the database release marker. The completed `PMT_1.26_to_1.27.sql` step and `PMT_1.26_to_1.27_All.sql` runner remain historical artifacts under `SQL/Migrations/Migration History/`. `PMT_SecurityRoleDefaultsVersion` remains `1.10`. Do not manually edit either version property.

All migrations, combined runners, and runbooks through Version 1.27 are historical artifacts under `SQL/Migrations/Migration History/`. The Version 1.20 step includes the one-time, `ProjectId`-authoritative correction of legacy generated-prefix Sprint and work-item codes under the PMT Project. Those scripts remain available for controlled reconstruction and audit only; do not rerun them on a current Version 1.27 installation.
...
From the deployed PMT Database Version 1.27 baseline forward, any change that affects the database, its SQL contract, production data, or the stability of BDO's deployed instance through database state or behavior must include a forward migration script from the immediately preceding deployed version.
...
If one deployment requires two or more versioned migration scripts, always create a combined `PMT_<start>_to_<end>_All.sql` file in `SQL/Migrations/`. A release may also provide a combined wrapper around one canonical step when deployment requires the same one-file SQLCMD entry point used in rehearsal, as Version 1.28 does. The individual version-step files remain the canonical migration history; the combined file is the operator-facing runner.
