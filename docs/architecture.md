...
The source tree and fresh-rebuild scripts represent PMT Database Version 1.22. BDO and every other known deployed instance remain on the Version 1.15 baseline until the combined Version 1.15-to-1.22 migration and matching application release are deployed. The combined migration must run successfully before the Version 1.22 binary starts against an existing database. Version 1.21 makes Root Cause Analysis synchronization one-way from Dev Task to Bug, copies Bug URLs one way to linked Bug Fix Dev Tasks, and prevents Developer-role users from moving Dev Tasks past `QA Passed`. Version 1.22 records successful password and invitation logins separately from `Users.RowVersion` and preserves each administrator's configured Role.
