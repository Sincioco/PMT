...
| Image annotation templates | `components/image-annotation.js` through the RTE image action | `GET /api/image-annotation/template-library`; `PUT /api/image-annotation/template-library` | `GetUserImageAnnotationTemplateLibraryAsync`; `SaveUserImageAnnotationTemplateLibraryAsync` | `[pmt].[GetUserImageAnnotationTemplateLibrary]`; `[pmt].[SaveUserImageAnnotationTemplateLibrary]`; one versioned JSON library in `[pmt].[UserImageAnnotationTemplateLibraries]` per authenticated effective user |
...
RTE image annotation output uses the existing upload contract. The original full-resolution upload remains the source image, while Apply uploads a self-contained SVG containing the rendered image, vector objects, and editable JSON metadata. The editor's oversized centered pasteboard is temporary and grows when an image or vector is dropped beyond its current bounds. An uncropped image uses its own movable bounds instead of a fixed canvas clip; an intentional non-destructive crop is stored as an image-relative visible region that moves and scales with the image. Apply calculates a tight SVG view box around the visible image plus annotation paint and excludes unused workspace. The stored RTE `<img>` points its `src` at the current SVG and retains the original upload in `data-pmt-annotation-source`; both URLs therefore remain ordinary database-backed rich-text references for Maintenance scanning. Uploaded SVG responses are sandboxed with a restrictive content security policy and `nosniff` header because users may also open the generated file directly.

Reusable annotation templates use a separate authenticated, per-user database contract so the same library follows a user between browsers and computers. `GET /api/image-annotation/template-library` reads the effective user's versioned library and `PUT` replaces it after browser and SQL validation. `[pmt].[UserImageAnnotationTemplateLibraries]` holds one JSON document per user, including at most 50 ordered templates and the optional Arrow and Rectangle drawing defaults. A template creates no upload or sidecar file: an included raster or SVG image is stored losslessly as its original image data inside the JSON, while rectangles, arrows, and text remain native editable members. Consequently, template images are outside the orphan-file scan and cannot become unreferenced upload candidates. The user foreign key owns library lifetime.

The inspector exposes Format, Template, and Objects as accessible tabs. Saving a selection captures its current native geometry and formatting; a mixed image-and-vector selection is retained as one reusable composition and represented by a large preview. Clicking a preview inserts a detached copy at the viewport center with fresh object identities and, for a multi-object template, a fresh group identity. The user can then ungroup and edit its members normally. Rename, update from the current selection, delete, and ordered move operations save the account library immediately, independently of Apply to RTE or Cancel. Existing canvas instances are snapshots rather than live links, so later template edits or deletion never rewrite objects already placed on a canvas. A single unlocked Arrow or Rectangle may supply the account's future drawing style, and either type can be reset to the PMT factory default.

The Objects tab is a compact projection of the same native canvas and SVG paint stack, not a separate object model. Its highest, frontmost painted item is the first row and its lowest, backmost item is the last row. Standalone objects render as tree rows; members that share a group identity render beneath one logical group row. The original source image remains the fixed bottom canvas layer: it can appear beneath a logical group row when grouped, but it cannot be deleted or reordered above annotations. Tree and canvas selection share one selected-ID set, so single and additive tree selection immediately updates canvas selection chrome and canvas selection immediately updates the tree. Object and group labels belong to the editable annotation metadata and participate in history, Apply, and reopen. Tree rename, multi-delete, copy, and paste therefore operate on the same native objects as their canvas equivalents.
