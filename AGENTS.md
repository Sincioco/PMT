...
- Treat PMT Database Version 1.22 as the deployed BDO baseline. As of July 16, 2026, the latest PMT release and current database schema are deployed to every known instance, so no new pre-1.22 upgrade compatibility is required. Every database-affecting or database-backed stability change must include a forward migration from the current deployed version. If one deployment requires more than one versioned migration, also provide a combined `PMT_<from>_to_<to>_All.sql` SQLCMD runner so the operator runs one file. Protecting BDO data and stability is the top priority; see `docs/database-versioning.md`.
...
- Treat `Requirements/*.txt` as historical planning and prompt records, never as implementation authorization. After the user explicitly authorizes work and the release is complete, update `scripts/release-notes.json`, regenerate the shared Release Notes/What's New data, and verify it as described in `docs/release-notes.md`.
...
- [Release Notes maintenance](docs/release-notes.md)
