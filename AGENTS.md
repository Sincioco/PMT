...
- Treat PMT Database Version 1.22 as the deployed BDO baseline until the active Version 1.23 migration is deployed. Every known instance was current at Version 1.22, so no new pre-1.22 upgrade compatibility is required. Every database-affecting or database-backed stability change must include a forward migration from the current deployed version. If one deployment requires more than one versioned migration, also provide a combined `PMT_<from>_to_<to>_All.sql` SQLCMD runner so the operator runs one file. Protecting BDO data and stability is the top priority; see `docs/database-versioning.md`.
